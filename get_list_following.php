...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (!isset($_POST['count']) || !isset($_POST['user_id']) || !isset($_POST['token']) || !isset($_POST['token'])) {
		$code = "1002";
		...
		if ($conn == null) {
			...
			if ($flag) {
				...
				$count = (int)$count;
				$index = (int)$index;
				$sql = "SELECT * FROM relative WHERE own_id = '$user_id' AND type = 'follow' ORDER BY id DESC LIMIT $index, $count";
